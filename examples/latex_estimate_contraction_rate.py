...
if __name__ == '__main__':
    ...
    C = (x[:, None] - y[None, :])**2
    scale = [-3., -2.5, -2., -1.5, -1., -0.5, -0., 0.5, 1.]
    rho = [0.1, 1., 10., 100.]
    colors = ['r', 'b', 'g', 'c']
    ...
    for r in range(len(rho)):
        ...
        rate_th = 2 * np.log10(1 + (eps / rho[r]))
        # rate_th = 0.
        plt.plot(scale, rate_s + rate_th, c=colors[r], linestyle='dashed', label='S')
        plt.plot(scale, rate_ti + rate_th, c=colors[r], label='TI')
